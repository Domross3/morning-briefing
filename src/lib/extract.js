...
export async function extractArticleBody(url, { userAgent, maxChars = 700 } = {}) {
  ...
  try {
    ...
    // HuggingFace paper pages have a fixed structure: the generic paragraph
    // harvester pulls in submission chrome ("Submitted by X · #1 Paper of the
    // day · Upvote 93 · Authors: ...") before the abstract. Special-case them
    // to extract only the abstract, and strip the "Paper page - " title prefix.
    if (/(^|\.)huggingface\.co$/i.test(safeHost(url)) && /\/papers\//.test(url)) {
      const hf = extractHuggingFacePaper(html);
      if (hf) return hf;
    }
...
// HuggingFace paper page → clean title + abstract only (no submission chrome,
// no author list). Returns null if the page doesn't look like a paper page so
// the caller falls back to generic extraction.
function extractHuggingFacePaper(html) {
  // Title: og:title is "Paper page - <Real Title>".
  const ogTitle =
    html.match(/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"']+)["']/i) ||
    html.match(/<meta[^>]+content=["']([^"']+)["'][^>]+property=["']og:title["']/i);
  let title = ogTitle ? stripHtml(ogTitle[1]).trim() : null;
  if (title) title = title.replace(/^paper page\s*[-–—:]\s*/i, "").trim();

  // Abstract: the text after the "Abstract</h2>" heading. Grab a generous
  // window and strip tags; the clip() call trims to a sentence boundary.
  const abMatch = html.match(/Abstract\s*<\/h2>([\s\S]{0,3000})/i);
  let abstract = abMatch ? compactWhitespace(stripHtml(abMatch[1])) : null;
  if (abstract) {
    // HF prepends an AI-generated one-sentence TL;DR, then "Generated by
    // <model>", then the full abstract. The TL;DR is a great concise summary —
    // keep it and drop the attribution + raw abstract when it's substantial.
    const genIdx = abstract.search(/\s*Generated by\s+\S/i);
    if (genIdx >= 60) {
      abstract = abstract.slice(0, genIdx).trim();
    } else {
      abstract = abstract.replace(/\s*Generated by\s+[^\s]+(\s+[^\s]+){0,4}?\s+(?=[A-Z])/i, " ").trim();
    }
    // Cut at the next page-section label if one leaked in.
    abstract = abstract
      .replace(/\s+(View arXiv page|View PDF|Add to collection|Community|Models? citing|Datasets? citing|Spaces? citing).*$/i, "")
      .trim();
  }

  if (!abstract || abstract.length < 60) return null;
  const summary = clip(abstract, 700);
  const fullWordCount = abstract.split(/\s+/).filter(Boolean).length;
  return { title, summary, fullWordCount };
}
